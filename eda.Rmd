...
```{r}
stores <- unique(train$store_id)

for(s in stores){
  store_i_sales_df <- train %>% 
    filter(store_id==s) %>%
    group_by(date) %>% 
    summarize(store_sales=sum(sales))
  
  store_i_sales_df$date <- as.Date(store_i_sales_df$date)
  
  fig <- store_i_sales_df %>%
    ggplot(aes(x=date, y=store_sales)) +
    geom_line() +
    labs(x='Date', y='Sales', title=paste('Daily Sales', s)) +
    scale_x_date(date_breaks='3 month', date_labels='%b %y') +
    theme_ipsum() +
    theme(axis.text.x=element_text(angle=45, hjust=1))
